...
class FreshersworldProvider(BaseProvider):
    """Provider for Freshersworld entry-level jobs and fresher opportunities via scraping."""
    ...
    async def search(self, query: str, **kwargs) -> list[RawJobData]:
        """Search Freshersworld for jobs matching the query."""
        ...
        try:
            ...
            logger.warning("Freshersworld search failed, returning empty results")
            return []
    ...
    def _parse_search_results(self, html: str, query: str) -> list[RawJobData]:
        ...
        for card in soup.select(
            ".job-row, .job-card, .job-list, .job-post, article, [class*='job'], "
            ".listing-item, .card-list-item, tr[class*='job']"
        ):
            try:
                title_el = card.select_one(
                    "h2 a, h3 a, .title a, [class*='title'] a, .job-title a, a[href*='job']"
                )
                company_el = card.select_one(
                    ".company-name, .company, .recruiter-name, [class*='company'], "
                    "[class*='recruiter']"
                )
                location_el = card.select_one(
                    ".location, .loc, [class*='location'], .place, .city"
                )
                salary_el = card.select_one(
                    ".salary, .sal, .package, [class*='salary'], [class*='package']"
                )
                experience_el = card.select_one(
                    ".experience, .exp, [class*='exp'], [class*='experience']"
                )
                skill_els = card.select(
                    ".skill, .tag, .key-skill, [class*='skill'], [class*='tag']"
                )
                date_el = card.select_one(
                    ".date, .posted, .time, [class*='date'], [class*='posted']"
                )
                desc_el = card.select_one(
                    ".description, .desc, .short-desc, [class*='description'], p"
                )
                qualification_el = card.select_one(
                    ".qualification, .education, .degree, [class*='qualification'], "
                    "[class*='education']"
                )
                ...
                qualification = qualification_el.get_text(strip=True) if qualification_el else None
                ...
                if url_el:
                    ...
                    if href:
                        url = href if href.startswith("http") else f"https://www.freshersworld.com{href}"

                job_type = "entry-level"
                if experience:
                    job_type = "internship" if "intern" in experience.lower() else "entry-level"
                ...
                jobs.append(RawJobData(
                    ...
                    skills=skills,
                    raw={
                        "source": "freshersworld", "query": query,
                        "experience": experience, "qualification": qualification,
                    },
                ))
            except Exception:
                logger.exception("Failed to parse Freshersworld job card")
    ...
    def _extract_job_id(self, card) -> str | None:
        ...
        if job_id:
            return str(job_id)
        url_el = card.select_one("a[href*='/job/'], a[href*='/jobs/']")
        if url_el:
            href = url_el.get("href", "")
