...
class FreshersworldJobProvider(BaseJobProvider):
    name = "freshersworld"
    display_name = "Freshersworld"
    description = "Freshersworld India job listings for fresh graduates"
    version = "1.0.0"
    ...
    def __init__(self, config: JobDiscoveryConfig) -> None:
        self.base_url = config.freshersworld.base_url
        self.page_size = config.freshersworld.page_size
    ...
    def _raw_to_posting(self, raw: dict) -> JobPosting:
        company_raw = raw.get("company", raw.get("employer", raw.get("recruiter", {}))) or {}
        company = CompanyInfo(
        ...
        title = raw.get("title", raw.get("jobTitle", raw.get("job_title", "Untitled Position")))
        description = raw.get("description", raw.get("jobDescription", raw.get("job_description", "")))
        url = raw.get("url", raw.get("jobUrl", raw.get("applyUrl", "")))
        ...
        return JobPosting(
            ...
            salary=salary,
            skills=raw.get("skills", raw.get("keySkills", raw.get("skillsRequired", []))) or [],
            posted_date=raw.get("postedDate", raw.get("createdDate", raw.get("posted_date"))),
            provider=self.name,
        )
    ...
    def _parse_salary(self, raw: dict) -> SalaryInfo | None:
        salary_min = raw.get("salaryMin", raw.get("salary_min", raw.get("minSalary", raw.get("min_amount"))))
        salary_max = raw.get("salaryMax", raw.get("salary_max", raw.get("maxSalary", raw.get("max_amount"))))
