...
class BambooHRJobProvider(BaseJobProvider):
    name = "bamboohr"
    display_name = "BambooHR"
    description = "BambooHR career portal job listings"
    version = "1.0.0"
    ...
    _subdomain: str = "default"

    def __init__(self, config: JobDiscoveryConfig) -> None:
        self.base_url = config.bamboohr.base_url
        self.page_size = config.bamboohr.page_size
        self._subdomain = config.bamboohr.company_subdomain
    ...
    async def search_jobs(self, request: JobSearchRequest) -> JobSearchResponse:
        ...
        path = f"/{self._subdomain}/v1/jobs"
    ...
    def _parse_response(self, data: dict, request: JobSearchRequest) -> JobSearchResponse:
        raw_results = data.get("jobs", data.get("results", data.get("data", [])))
    ...
    def _raw_to_posting(self, raw: dict) -> JobPosting:
        company_raw = raw.get("company", raw.get("department", {})) or {}
        company = CompanyInfo(
        ...
        return JobPosting(
            provider_job_id=str(raw.get("id", raw.get("jobId", raw.get("job_id", "")))),
            title=title,
            company=company,
            location=location,
            ...
            posted_date=raw.get("postedDate", raw.get("createdDate", raw.get("datePosted", raw.get("postedAt")))),
            provider=self.name,
        )
    ...
    def _parse_salary(self, raw: dict) -> SalaryInfo | None:
        salary_min = raw.get("salaryMin", raw.get("minSalary", raw.get("salaryMinimum", raw.get("salaryFrom"))))
        salary_max = raw.get("salaryMax", raw.get("maxSalary", raw.get("salaryMaximum", raw.get("salaryTo"))))
        ...
        compensation = raw.get("compensation", raw.get("payRate", ""))
        if isinstance(compensation, str) and compensation:
            try:
                cleaned = compensation.replace("$", "").replace(",", "").strip()
                parts = cleaned.split("-")
    ...
    def _normalize_employment(self, raw: dict) -> EmploymentType:
        ...
        if any(kw in emp_str for kw in ("full", "permanent", "regular")):
    ...
    async def _fetch_provider_status(self) -> object:
        data = await self._client.get(f"/{self._subdomain}/v1/jobs", params={"limit": 1})
        return data
