...
class FounditJobProvider(BaseJobProvider):
    name = "foundit"
    display_name = "Foundit"
    description = "Foundit (formerly Monster India) job listings"
    version = "1.0.0"
    ...
    def __init__(self, config: JobDiscoveryConfig) -> None:
        self.base_url = config.foundit.base_url
        self.page_size = config.foundit.page_size
    ...
    def _raw_to_posting(self, raw: dict) -> JobPosting:
        company_raw = raw.get("company", raw.get("employer", raw.get("companyInfo", {}))) or {}
        company = CompanyInfo(
    ...
    def _parse_salary(self, raw: dict) -> SalaryInfo | None:
        salary_min = raw.get("salaryMin", raw.get("salary_min", raw.get("minSalary", raw.get("salaryMinimum"))))
        salary_max = raw.get("salaryMax", raw.get("salary_max", raw.get("maxSalary", raw.get("salaryMaximum"))))
