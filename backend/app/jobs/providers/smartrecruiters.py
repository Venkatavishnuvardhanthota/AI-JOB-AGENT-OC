...
class SmartRecruitersJobProvider(BaseJobProvider):
    ...
    description = "SmartRecruiters career portal job listings"
    version = "1.0.0"
    ...
    def __init__(self, config: JobDiscoveryConfig) -> None:
        self.base_url = config.smartrecruiters.base_url
        self.page_size = config.smartrecruiters.page_size
        self._company_id = config.smartrecruiters.company_id
    ...
    async def search_jobs(self, request: JobSearchRequest) -> JobSearchResponse:
        ...
        path = f"/companies/{self._company_id}/postings"
    ...
    def _parse_response(self, data: dict, request: JobSearchRequest) -> JobSearchResponse:
        raw_results = data.get("content", data.get("results", data.get("data", [])))
        total = data.get("total", data.get("totalFound", len(raw_results)))
    ...
    def _raw_to_posting(self, raw: dict) -> JobPosting:
        ...
        title = raw.get("title", raw.get("name", raw.get("positionName", "Untitled Position")))
        description = raw.get("description", raw.get("jobDescription", raw.get("descriptionPlain", "")))
        url = raw.get("url", raw.get("applyUrl", ""))

        return JobPosting(
            provider_job_id=str(raw.get("id", raw.get("postingId", raw.get("uuid", "")))),
            title=title,
            company=company,
            location=location,
            ...
            posted_date=raw.get("postedDate", raw.get("createdDate", raw.get("publicationDate"))),
            provider=self.name,
        )
    ...
    def _parse_location(self, raw: dict) -> LocationInfo:
        ...
        if isinstance(loc_raw, dict):
            ...
            country = loc_raw.get("country", loc_raw.get("countryCode", raw.get("country", "")))
    ...
    def _parse_salary(self, raw: dict) -> SalaryInfo | None:
        ...
        salary_str = raw.get("salary", raw.get("compensation", ""))
    ...
    async def _fetch_provider_status(self) -> object:
        data = await self._client.get(f"/companies/{self._company_id}/postings", params={"limit": 1})
        return data
