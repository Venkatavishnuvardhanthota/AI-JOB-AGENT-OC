...
class InternshalaJobProvider(BaseJobProvider):
    name = "internshala"
    display_name = "Internshala"
    description = "Internshala internships and job listings"
    version = "1.0.0"
    ...
    def __init__(self, config: JobDiscoveryConfig) -> None:
        self.base_url = config.internshala.base_url
        self.page_size = config.internshala.page_size
    ...
    async def search_jobs(self, request: JobSearchRequest) -> JobSearchResponse:
        ...
        data = await self._client.get("/api/internships", params=params)
    ...
    def _parse_response(self, data: dict, request: JobSearchRequest) -> JobSearchResponse:
        raw_results = data.get("internships", data.get("jobs", data.get("results", [])))
    ...
    def _raw_to_posting(self, raw: dict) -> JobPosting:
        company_raw = raw.get("company", raw.get("organization", raw.get("employer", {}))) or {}
        company = CompanyInfo(
        ...
        title = raw.get("title", raw.get("name", raw.get("profile", "Untitled Position")))
        description = raw.get("description", raw.get("about", ""))
        url = raw.get("url", raw.get("applyUrl", raw.get("link", "")))

        is_internship = raw.get("type") == "internship" or "intern" in title.lower()

        return JobPosting(
            provider_job_id=str(raw.get("id", raw.get("internshipId", raw.get("postId", "")))),
            title=title,
            company=company,
            location=location,
            ...
            posted_date=raw.get("postedDate", raw.get("createdDate", raw.get("posted_on"))),
            provider=self.name,
        )

    def _parse_location(self, raw: dict) -> LocationInfo:
        loc_raw = raw.get("location", raw.get("locations", raw.get("place", {})))
    ...
    def _parse_salary(self, raw: dict) -> SalaryInfo | None:
        salary_min = raw.get("salaryMin", raw.get("stipendMin", raw.get("minSalary", raw.get("min_amount"))))
        salary_max = raw.get("salaryMax", raw.get("stipendMax", raw.get("maxSalary", raw.get("max_amount"))))
        ...
        salary_str = raw.get("stipend", raw.get("salary", ""))
        if isinstance(salary_str, str) and salary_str:
            try:
                ...
                return SalaryInfo(min_amount=min_sal, max_amount=max_sal, currency="INR", period="monthly")
    ...
    async def _fetch_provider_status(self) -> object:
        data = await self._client.get("/api/internships", params={"limit": 1})
        return data
