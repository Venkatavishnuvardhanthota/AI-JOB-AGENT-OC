...
class RecruiteeJobProvider(BaseJobProvider):
    name = "recruitee"
    display_name = "Recruitee"
    description = "Recruitee career portal job listings"
    version = "1.0.0"
    supports_pagination = True
    supports_filters = True

    base_url = "https://api.recruitee.com/c"
    api_key_scheme = ""
    page_size = 20

    _company_id: str = "default"

    def __init__(self, config: JobDiscoveryConfig) -> None:
        self.base_url = config.recruitee.base_url
        self.page_size = config.recruitee.page_size
        self._company_id = config.recruitee.company_id
    ...
    def _parse_response(self, data: dict, request: JobSearchRequest) -> JobSearchResponse:
        raw_results = data.get("jobs", data.get("offers", data.get("results", [])))
    ...
    def _raw_to_posting(self, raw: dict) -> JobPosting:
        company_raw = raw.get("company", raw.get("department", raw.get("organization", {}))) or {}
        company = CompanyInfo(
            name=company_raw.get("name", raw.get("companyName", raw.get("company_name", "Unknown Company"))),
            description=company_raw.get("description"),
        )

        location = self._parse_location(raw)
        salary = self._parse_salary(raw)
        title = raw.get("title", raw.get("position", raw.get("name", "Untitled Position")))
        description = raw.get("description", raw.get("offerDescription", raw.get("descriptionHtml", "")))
        url = raw.get("url", raw.get("applyUrl", raw.get("careerPageUrl", "")))

        return JobPosting(
            provider_job_id=str(raw.get("id", raw.get("offerId", raw.get("slug", "")))),
            title=title,
            company=company,
            location=location,
            ...
            salary=salary,
            skills=raw.get("skills", raw.get("tags", raw.get("skillTags", []))) or [],
            posted_date=raw.get("postedDate", raw.get("createdAt", raw.get("created_at", raw.get("publishedAt")))),
            provider=self.name,
        )

    def _parse_location(self, raw: dict) -> LocationInfo:
        loc_raw = raw.get("location", raw.get("locations", raw.get("office", {})))
    ...
    def _parse_salary(self, raw: dict) -> SalaryInfo | None:
        salary_min = raw.get("salaryMin", raw.get("minSalary", raw.get("salaryFrom", raw.get("salaryMinimum"))))
        salary_max = raw.get("salaryMax", raw.get("maxSalary", raw.get("salaryTo", raw.get("salaryMaximum"))))
        ...
        salary_str = raw.get("salary", raw.get("compensation", raw.get("payRange", "")))
    ...
    def _normalize_employment(self, raw: dict) -> EmploymentType:
        emp_type = raw.get("employmentType", raw.get("type", raw.get("employment_type", raw.get("contractType", ""))))
        if isinstance(emp_type, dict):
            emp_type = emp_type.get("name", "")
        emp_str = str(emp_type).lower() if emp_type else ""
        if any(kw in emp_str for kw in ("full", "permanent", "regular", "b2b")):
            return EmploymentType.FULL_TIME
        if "part" in emp_str:
            return EmploymentType.PART_TIME
        if any(kw in emp_str for kw in ("contract", "temporary")):
    ...
    async def _fetch_provider_status(self) -> object:
        data = await self._client.get(f"/{self._company_id}/jobs", params={"limit": 1})
        return data
