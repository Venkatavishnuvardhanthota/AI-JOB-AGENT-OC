...
class PackageValidator:
    def validate(
        ...
        if cover_letter and job:
            name_ok, name_issues = self._check_company_name_consistency(
                cover_letter, job
            )
    ...
    @staticmethod
    def _check_profile_job_consistency(
        profile: UserIntelligenceProfile, job: JobPosting
    ) -> list[str]:
        issues: list[str] = []
        title = job.title or ""
        ...
        profile_has_seniority = any(
            kw in profile_current_role for kw in seniority_keywords
        )
    ...
    @staticmethod
    def _check_job_consistency(
        match_result: MatchResult, job: JobPosting
    ) -> tuple[bool, list[str]]:
        issues: list[str] = []
    ...
    @staticmethod
    def _check_resume_job_consistency(
        resume: OptimizedResume, job: JobPosting
    ) -> tuple[bool, list[str]]:
        issues: list[str] = []
    ...
    def _check_company_name_consistency(
        cover_letter: GeneratedCoverLetter, job: JobPosting
    ) -> tuple[bool, list[str]]:
        issues: list[str] = []
        cl_company = (
            cover_letter.personalization.company_name or ""
        ).lower().strip()
        job_company = (
        ...
        if cl_company and job_company and cl_company != job_company:
            issues.append(
                f"Company name mismatch: cover letter has '{cl_company}', "
                f"job has '{job_company}'."
            )
            return False, issues
        return True, issues
