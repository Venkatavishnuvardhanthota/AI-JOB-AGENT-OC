...
class ATSService:
    ...
    def login(self, provider_name: str, request: ATSLoginRequest) -> ATSLoginResult:
        ...
        browser_id = self._browser.create_browser(headless=self._config.default_headless)
        try:
            context = self._browser.create_context(browser_id["id"])
            self._browser.create_session(browser_id["id"], context["id"])
            page = None
            return provider.login(page, request)
        finally:
            self._browser.close_browser(browser_id["id"])
    ...
    def navigate(
        ...
        try:
            ...
            self._browser.create_session(browser_id["id"], context["id"])
            page = None
            return provider.navigate(page, request)
        finally:
            self._browser.close_browser(browser_id["id"])
    ...
    def find_jobs(
        ...
        try:
            ...
            self._browser.create_session(browser_id["id"], context["id"])
            page = None
            return provider.find_job(page, request)
        finally:
            self._browser.close_browser(browser_id["id"])
    ...
    def open_application(
        ...
        try:
            ...
            self._browser.create_session(browser_id["id"], context["id"])
            page = None
            result = provider.open_application(page, request)
            if self._config.screenshot_on_error and not result.success:
                result.screenshot_path = self._browser.take_failure_screenshot(
                    page,
                    f"ats_{provider_name}_application_error",
                )
            return result
        finally:
            self._browser.close_browser(browser_id["id"])
